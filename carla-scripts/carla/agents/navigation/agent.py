...
import carla
from carla import World, Map, ActorList, Actor, Vehicle
...
class Agent(object):
    ...
    def _is_light_red(self, lights_list: List[Actor]):
        """
        ...
        """
        if False and (self._map.name == 'Town01' or self._map.name == 'Town02'):
            return self._is_light_red_europe_style(lights_list)
        else:
            return self._is_light_red_us_style(lights_list)

    def _is_light_red_europe_style(self, lights_list: List[Actor]):
        """
    ...
    def _is_light_red_us_style(self, lights_list: List[Actor], debug=False):
        """
